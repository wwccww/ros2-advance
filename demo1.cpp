...
int main(int argc, char ** argv){
  ...
  */
  ...
  // step 5: move down to place 
  std::vector<double> place_position = target_position_3;
  place_position[2] = 0.16; 
  RCLCPP_INFO(LOGGER, "Lowering arm to place the object...");
  success = manipulator_plan_and_execute(move_group, place_position, {0, M_PI, M_PI_2});
  if(!success){
    rclcpp::shutdown();
    return 0;
  }
  ...
  // step 7: retract 
  std::vector<double> retract_position = target_position_3;
  retract_position[2] += 0.15; 
  success = manipulator_plan_and_execute(move_group, retract_position, {0, M_PI, M_PI_2});
